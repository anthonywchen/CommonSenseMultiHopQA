...
def main(data_file, prediction_file, bleu1_file, bleu4_file, rouge_file, meteor_file, cider_file, bert_score_file):
	...
	assert len(data_lines) == len(bert_score_lines)
 	...
 	with open(output_file, 'w') as writer:
		...
		for data_num in range(len(data_lines)):
			...
			data_dict['rogue'] = rouge_lines[data_num]
...
if __name__ == '__main__':
	...
	parser.add_argument('--data_file', type=str, required=True, help='path to file with generated answers')
	parser.add_argument('--prediction_file', type=str, required=True, help='path to file of reference answers')

	parser.add_argument('--bleu1_file', type=str, required=True, default=None, help='path to file of (second) reference answers')
	parser.add_argument('--bleu4_file', type=str, required=True, default=None, help='path to file of (second) reference answers')
	parser.add_argument('--rouge_file', type=str, required=True, default=None, help='path to file of (second) reference answers')
	parser.add_argument('--meteor_file', type=str, required=True, default=None, help='path to file of (second) reference answers')
	parser.add_argument('--cider_file', type=str, required=True, default=None, help='path to file of (second) reference answers')
	parser.add_argument('--bert_score_file', type=str, required=True, default=None, help='path to file of (second) reference answers')

	args = parser.parse_args()
	main(args.data_file, args.prediction_file, args.bleu1_file, args.bleu4_file, \
		 args.rouge_file, args.meteor_file, args.cider_file, args.bert_score_file)